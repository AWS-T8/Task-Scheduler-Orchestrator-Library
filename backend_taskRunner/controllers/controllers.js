...
const retryTask = (req, res, task) => {
	...
	producer.send(payloads, function (err, data) {
		if (err) {
			...
			return res
				.status(500)
				.json({ message: 'Error Occured In Producing!' });
		} else {
			console.log('Message Sent!');
			return res.status(201).json({ message: 'Retry Scheduled!' });
		}
	});
};
...
exports.saveTask = async (req, res) => {
	...
	try {
		...
		let retryCount = parseInt(task.retryCount);
		if (newStatus === 'failed' && retryCount > 0) {
			retryCount--;
			task.retryCount = retryCount.toString();
